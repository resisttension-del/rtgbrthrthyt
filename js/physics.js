...
// QUICK SYNC: copy Three.js physics camera -> PlayCanvas camera entity
(function syncPhysicsCamToPc() {
// ----- Quaternion-based rotation smoothing (replace your rotation block with this) -----
try {
  // Make sure THREE is available
  if (typeof THREE === "undefined") throw new Error("THREE required for quaternion rotation sync");

  // Target quaternion from physics camera (Three.js camera)
  const targetQuat = new THREE.Quaternion().setFromEuler(physCam.rotation); // physCam.rotation.order should already be 'YXZ'

  // Read current PlayCanvas Euler (degrees) safely
  let currRotDeg = { x: 0, y: 0, z: 0 };
  try {
    if (typeof window.camera.getLocalEulerAngles === "function") {
      const e = window.camera.getLocalEulerAngles();
      currRotDeg = { x: e.x, y: e.y, z: e.z };
    } else {
      // fallback: try getEuler (some codepaths)
      const e = window.camera.getEuler && window.camera.getEuler();
      if (e) currRotDeg = { x: e.x * 180/Math.PI, y: e.y * 180/Math.PI, z: e.z * 180/Math.PI };
    }
  } catch (e) { /* ignore and use zeros */ }

  // Convert current PlayCanvas Euler -> radians -> quaternion using same order ('YXZ')
  ...
  // Angle between quaternions (in degrees). Use dot-clamp to avoid numeric issues.
  const dot = Math.max(-1, Math.min(1, currQuat.dot(targetQuat)));
  const angleDeg = (2 * Math.acos(Math.abs(dot))) * (180 / Math.PI); // 0..360

  // If very large angle, snap. Otherwise slerp for smooth rotation.
  ...
  console.warn("quat-rotation-sync failed:", err);
}
