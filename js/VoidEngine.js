...
export function voidEngine({ width = 1280, height = 720 } = {}) {
  ...
  const api = {
    ...
    options: {
      strictNearClip: true,
      // GL depth-pass settings tuned for low-power devices:
      useGLDepthPass: true,
      glDepthWidth: 128,   // default offscreen GL buffer width (tiny)
      glDepthHeight: 72,   // default offscreen GL buffer height (tiny)
      maxTriangulation: 4000, // safe limit (triangles); if exceeded we fall back to painter
      // painter fallbacks:
  ...
  // Minimal world-sampling (same as before)
  function sampleWorldPointsFor(obj, geom) {
    const worldPoints = [];
    if (geom && geom.boundingBox) {
      const bb = geom.boundingBox; const min = bb.min; const max = bb.max;
      const corners = [
      ...
      for (let c of corners) { tmpVec.set(c[0],c[1],c[2]).applyMatrix4(obj.matrixWorld); worldPoints.push(tmpVec.clone()); }
      ...
      const posAttr = geom.attributes.position; const count = posAttr.count;
      ...
      obj.getWorldPosition(tmpPos); worldPoints.push(tmpPos.clone());
    }
    return worldPoints;
  }

  // ---------- WebGL offscreen helper (very small GL drawbuffer) ----------
  // returns {supported, gl, canvas, program, attribLocs, uniformLocs}
  function createOffscreenGL(w, h) {
    try {
      const off = document.createElement('canvas');
      off.width = w; off.height = h;
      // try webgl2 then webgl
      const gl = off.getContext('webgl2', { antialias:false, preserveDrawingBuffer:true }) || off.getContext('webgl', { antialias:false, preserveDrawingBuffer:true });
      if (!gl) return { supported: false };

      // simple shaders: vertex expects vec3 clip-space pos (NDC) and fragment uses uniform color
      const vsSrc = (gl instanceof WebGL2RenderingContext ? `#version 300 es
      in vec3 a_pos;
      void main(){ gl_Position = vec4(a_pos, 1.0); }` : `
      attribute vec3 a_pos;
      void main(){ gl_Position = vec4(a_pos, 1.0); }`);

      const fsSrc = (gl instanceof WebGL2RenderingContext ? `#version 300 es
      ...
      function compile(src, type) {
        ...
        if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) { const info = gl.getShaderInfoLog(s); gl.deleteShader(s); throw new Error(info); }
        return s;
      }
      ...
      const attribLoc = { a_pos: gl.getAttribLocation(prog, 'a_pos') };
      ...
      return { supported:true, gl, canvas:off, program:prog, attribLoc, uniformLoc, vbo };
    } catch (e) {
      return { supported:false };
    }
  }

  // drawDrawable: same shape drawing as earlier (2D canvas)
  function drawDrawable(d, camera) {
    ...
    if (!color && obj.material && obj.material.color) {
      try { color = obj.material.color.getStyle ? obj.material.color.getStyle() : (`#${obj.material.color.getHexString()}`); } catch(e) { color = obj.userData?.color || 'white'; }
    }
    ...
    if (d.type === 'image' && d.mapImage && d.mapImage.width) {
      ...
      ctx.globalAlpha = 0.6; ctx.lineWidth = Math.max(1, 2 - (avgDist * 0.001));
      ...
      ctx.globalAlpha = 1 - Math.min(0.9, avgDist * 0.002); ctx.stroke();
    } else if (d.type === 'rect') {
      const size = d.sizePx ?? Math.max(2, Math.round(12 * (1 / Math.max(0.1, avgDist * 0.05))));
      const x = (d.sx || d.sx === 0) ? d.sx : 0; const y = (d.sy || d.sy === 0) ? d.sy : 0;
      ctx.save(); ctx.globalAlpha = Math.max(0.5, Math.min(1, 1 - (avgDist * 0.002))); ctx.fillStyle = color;
  ...
  api.render = function(scene, camera) {
    ...
    ctx.save(); ctx.setTransform(1,0,0,1,0,0); ctx.fillStyle = `rgba(${rr},${gg},${bb},${c.alpha})`; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.restore();

    camera.updateMatrixWorld(); if (camera.updateProjectionMatrix) camera.updateProjectionMatrix();
    const camInv = tmpMat.copy(camera.matrixWorld).invert();

    // gather drawables (same approach)
    const drawables = [];
    scene.traverse(obj => {
      if (!obj.visible) return; if (obj.isCamera || obj.isLight) return;
      obj.getWorldPosition(tmpPos); proj.copy(tmpPos).project(camera);
      ...
      function makeProjectedForMesh(obj, geom) {
        ...
        const distNear = Math.min(...dists); const distFar = Math.max(...dists);
        ...
        for (let i = 0; i < worldPoints.length; i++) {
          const camPt = camSpacePts[i]; const wp = worldPoints[i];
          if (camPt.z <= nearZ && camPt.z >= farZ) { proj.copy(wp).project(camera); pts2d.push({ x:(proj.x*0.5+0.5)*canvas.width, y:(-proj.y*0.5+0.5)*canvas.height, ndcZ:proj.z }); }
        }
        // add near-plane intersections
        for (let i = 0; i < worldPoints.length; i++) for (let j = i+1; j < worldPoints.length; j++) {
          ...
          if ((z1 <= nearZ && z2 > nearZ) || (z2 <= nearZ && z1 > nearZ)) {
            ...
            proj.copy(ip).project(camera); pts2d.push({ x:(proj.x*0.5+0.5)*canvas.width, y:(-proj.y*0.5+0.5)*canvas.height, ndcZ:proj.z });
          }
        }
        if (!api.options.strictNearClip && pts2d.length === 0) { proj.copy(tmpPos).project(camera); pts2d.push({ x:(proj.x*0.5+0.5)*canvas.width, y:(-proj.y*0.5+0.5)*canvas.height, ndcZ:proj.z }); }
      ...
      if (mapImage) {
        ...
        const dists = worldPoints.map(wp => camera.position.distanceTo(wp));
        drawables.push({ type:'image', obj, sx, sy, distNear:Math.min(...dists), distFar:Math.max(...dists), projZ:proj.z, mapImage });
        return;
      }

      if (obj.isMesh && obj.geometry) {
        const geom = obj.geometry; if (!geom.boundingBox) geom.computeBoundingBox && geom.computeBoundingBox(); if (!geom.boundingSphere) geom.computeBoundingSphere && geom.computeBoundingSphere();
        const { pts2d, distNear, distFar } = makeProjectedForMesh(obj, geom);
        if (pts2d.length === 0) {
          if (alwaysRender || obj.userData?.forceMarker) {
            const cx = Math.max(0, Math.min(canvas.width, sx)); const cy = Math.max(0, Math.min(canvas.height, sy));
        ...
        if (hull.length >= 3) drawables.push({ type:'poly', obj, pts:hull, distNear, distFar, projZ:proj.z });
        else if (hull.length === 2) drawables.push({ type:'line', obj, pts:hull, distNear, distFar });
        else { const p = hull[0] || pts2d[0]; if (alwaysRender || obj.userData?.forceMarker) drawables.push({ type:'rect', obj, sx:p.x, sy:p.y, distNear, distFar, sizePx: obj.userData?.markerSizePx ?? 6 }); }
        return;
      }

      if (obj.userData?.forceMarker) drawables.push({ type:'rect', obj, sx, sy, distNear:centerDist, distFar:centerDist, sizePx: obj.userData?.markerSizePx ?? 6 });
    });
    ...
    for (const d of drawables) {
      ...
      if (alpha >= 0.999 && !d.obj.userData?.forceTranslucent) opaque.push(d); else translucent.push(d);
    }

    function depthCmp(a,b) {
      const aFar = a.distFar ?? a.dist ?? 0, bFar = b.distFar ?? b.dist ?? 0;
      if (aFar !== bFar) return bFar - aFar;
      const aNear = a.distNear ?? a.dist ?? 0, bNear = b.distNear ?? b.dist ?? 0;
      if (aNear !== bNear) return bNear - aNear;
      const aZ = (a.projZ !== undefined ? a.projZ : 1), bZ = (b.projZ !== undefined ? b.projZ : 1);
      return bZ - aZ;
    }

    // cheap heuristic: if extremely many drawables, do painter to avoid tri explosion
    ...
    // If GL depth pass disabled or not supported -> painter fallback
    let glState = null;
    ...
    if (api.options.useGLDepthPass) glState = createOffscreenGL(wGL, hGL);
    if (!glState || !glState.supported) {
      // painter
      opaque.sort(depthCmp); for (const d of opaque) drawDrawable(d, camera);
      translucent.sort((a,b) => -depthCmp(a,b)); for (const d of translucent) drawDrawable(d, camera);
      return;
    ...
    // compute total triangles to ensure we won't blow up
    let triCount = 0;
    ...
    if (triCount > api.options.maxTriangulation) {
      // too many triangles -> fallback painter
      opaque.sort(depthCmp); for (const d of opaque) drawDrawable(d, camera);
      translucent.sort((a,b) => -depthCmp(a,b)); for (const d of translucent) drawDrawable(d, camera);
      return;
    }

    // DO GL PASS: draw opaque convex hulls into offscreen GL canvas using clip-space coords (NDC)
    const { gl, canvas: glCanvas, program, attribLoc, uniformLoc, vbo } = glState;
    gl.viewport(0,0, glCanvas.width, glCanvas.height);
    gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
    gl.useProgram(program);
    gl.clearColor(rr/255, gg/255, bb/255, c.alpha);
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);

    // per-object draw
    for (const d of opaque) {
      // compute polygon vertices in clip-space (vec3: x,y,z)
      let verts = [];
      if (d.type === 'poly' && d.pts && d.pts.length >= 3) {
        // use pts (screen coords) -> convert back to NDC x/y and use ndcZ for z
        for (let p of d.pts) {
        ...
        // approximate quad centered at d.sx,d.sy using projZ
      ...
      for (let i = 1; i < verts.length - 1; i++) {
        triVerts.push(...verts[0], ...verts[i], ...verts[i+1]);
      }
      if (triVerts.length === 0) continue;

      // upload vertex data
      const data = new Float32Array(triVerts);
      gl.bufferData(gl.ARRAY_BUFFER, data, gl.STATIC_DRAW);
      gl.enableVertexAttribArray(attribLoc.a_pos);
      // since in WebGL1 attribute size must be 3 floats
      gl.vertexAttribPointer(attribLoc.a_pos, 3, gl.FLOAT, false, 0, 0);

      // compute object color from material/userData
      ...
      // parse cssColor to rgba bytes using a 1px canvas trick (cheap)
      let rgba = [255,255,255, Math.round(255 * (d.obj.userData?.opacity ?? d.obj.material?.opacity ?? 1))];
      try {
        const tcan = document.createElement('canvas'); tcan.width = tcan.height = 1;
        const tctx = tcan.getContext('2d');
        tctx.clearRect(0,0,1,1);
        tctx.fillStyle = cssColor || 'white';
        tctx.fillRect(0,0,1,1);
        const p = tctx.getImageData(0,0,1,1).data;
        rgba = [p[0], p[1], p[2], rgba[3]];
      } catch(e) { /* ignore parsing failure: use fallback */ }

      // set uniform color (normalized)
      gl.uniform4f(uniformLoc.u_color, rgba[0]/255, rgba[1]/255, rgba[2]/255, rgba[3]/255);

      // draw
      gl.drawArrays(gl.TRIANGLES, 0, data.length / 3);
    }

    // finally blit the small GL canvas into our 2D canvas scaled (this is hardware-accelerated in most browsers)
    try {
      ctx.save();
      ...
      // if drawImage fails for any reason, fallback to painter
      opaque.sort(depthCmp); for (const d of opaque) drawDrawable(d, camera);
    }

    // draw translucent on top (back-to-front)
