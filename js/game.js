...
function voidEngine({ width = 1280, height = 720 } = {}) {
  ...
  function isVisible(camera, obj, scene) {
    // Raycaster from camera to object center
    ...
    // Find nearest intersect that is not the target object or its descendants
    for (let inter of intersects) {
      if (inter.object === obj || obj.children.includes(inter.object)) {
        // This is the target object, so no occlusion before it
        return true;
      }
      // If intersect distance is less than distance to target, occluded
      const distToTarget = origin.distanceTo(target);
      if (inter.distance < distToTarget) {
        // Hit something before the object: occluded
        return false;
      }
    }
    // If none occlude, visible
    return true;
  }
  ...
  const api = {
    ...
    render(scene, camera) {
      ...
      scene.traverse((obj) => {
        ...
        // Check occlusion by walls: skip object if occluded
        if (!isVisible(camera, obj, scene) && !obj.userData?.alwaysRender) return;
