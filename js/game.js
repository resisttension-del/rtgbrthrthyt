...
// Replace your existing syncPhysicsCamToPcWithSnapping with this complete, self-contained block
(function syncPhysicsCamToPcWithSnapping() {
  try {
    if (!window.playcanvasApp) return;
    if (!physicsController) return;
    if (!physicsController.camera) return; // nothing to sync from
    if (!window.camera || typeof window.camera.setLocalPosition !== "function") return;

    const physCam = physicsController.camera; // <--- ensured defined here
    ...
    // read current PlayCanvas camera position (try a few getters)
    let currPosVec = { x: 0, y: 0, z: 0 };
    try {
      if (typeof window.camera.getLocalPosition === "function") {
        const p = window.camera.getLocalPosition();
    ...
    // ---- Quaternion-based rotation smoothing (avoids Euler seam flips) ----
    try {
      if (typeof THREE === "undefined") throw new Error("THREE is required for quaternion rotation sync");

      // Build target quaternion from physics camera's Euler
      // (assumes physCam.rotation is a THREE.Euler with the correct order, e.g., 'YXZ')
      ...
      try {
        ...
        // ignore read errors, continue with zeros
      }

      const DEG2RAD = Math.PI / 180;
      // Important: use same Euler order as physics camera (you earlier set 'YXZ')
      ...
      if (angleDeg > SNAP_ANGLE_DEG || physicsController._forceCameraSnap) {
        // Snap instantly to target rotation
      ...
      console.warn("quat-rotation-sync failed:", rotErr);
    }
