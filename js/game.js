...
function _disposeOtherCanvases(container, keepId = "game-canvas") {
  try {
    const canvases = Array.from(container.querySelectorAll("canvas"));
    canvases.forEach(c => {
  ...
  } catch (e) { console.warn("_disposeOtherCanvases error:", e); }
}

// ---------- Robust PlayCanvas scene initializer ----------
export async function initSceneCrocodilosConstruction() {
  ...
  // 0) remove other canvases/contexts that may create conflicting GL contexts
  _disposeOtherCanvases(container, "game-canvas");

  // fallback fixed internal resolution
  ...
  if (!canvas) {
    ...
    // set drawingbuffer size *attributes* (critical)
  ...
  // 2) create or reuse the PlayCanvas app (single global instance)
  ...
  if (!pcApp) {
    ...
    // fixed-res config
    try { pcApp.setCanvasFillMode(pc.FILLMODE_NONE); pcApp.setCanvasResolution(pc.RESOLUTION_FIXED); } catch (e) {}

    // enforce drawingbuffer size & cap pixel ratio
    ...
    // ensure canvas/drawbuffer sizes match our fixed resolution
    try { pcApp.resizeCanvas(window.FIXED_WIDTH, window.FIXED_HEIGHT); } catch (e) {}
    console.log("Reusing existing PlayCanvas app.");
  }

  // Small delayed re-assertion (works around engine race where drawingbuffer can be zero momentarily)
  setTimeout(() => {
    try {
      const gd = pcApp.graphicsDevice;
      if (gd && gd.canvas) {
        // Force attributes on the canvas the GL device actually uses
        gd.canvas.width = window.FIXED_WIDTH;
        gd.canvas.height = window.FIXED_HEIGHT;
        gd.canvas.style.width = `${container.clientWidth}px`;
        gd.canvas.style.height = `${container.clientHeight}px`;
      }
      try { pcApp.resizeCanvas(window.FIXED_WIDTH, window.FIXED_HEIGHT); } catch (e) {}
    } catch (e) { /* ignore */ }
    ...
    try {
      const gd = pcApp.graphicsDevice;
      const gl = gd && gd.gl;
      console.log("Post-start canvas sizes:",
        gd && gd.canvas && gd.canvas.width, "x", gd && gd.canvas && gd.canvas.height,
        "GL drawingBuffer:", gl && gl.drawingBufferWidth, "x", gl && gl.drawingBufferHeight);
    } catch (e) {}
  }, 50);

  // 3) camera entity - always recreate to keep names consistent for other code
  try {
    const prevCam = pcApp.root.findByName("game-camera");
    if (prevCam) prevCam.destroy();
  } catch (e) {}
  const cameraEnt = new pc.Entity("game-camera");
  cameraEnt.addComponent("camera", { clearColor: new pc.Color(0,0,0), fov:60, nearClip:0.1, farClip:2000 });
  ...
  // 4) directional light (best effort)
  try {
    const prevLight = pcApp.root.findByName("map-sun");
    if (prevLight) prevLight.destroy();
    const sunEnt = new pc.Entity("map-sun");
    sunEnt.addComponent("light", { type: "directional", color: new pc.Color(1,1,1), intensity: 0.9, castShadows:false });
    sunEnt.setLocalEulerAngles(50, 30, 0);
    pcApp.root.addChild(sunEnt);
    window.hemi = sunEnt;
  } catch (e) { console.warn("failed to create light", e); }

  // 5) disable previous three composer references if present
  try { composer = null; renderPass = null; window.composer = null; window.renderPass = null; } catch (e) {}

  // 6) load map and BVH collider (keeps your original createCrocodilosConstruction API)
  let spawnPoints = [];
  try {
  ...
  // 7) set initial player position if physics controller supports it
  ...
  function onWindowResize() {
    ...
    try { pcApp.resizeCanvas(window.FIXED_WIDTH, window.FIXED_HEIGHT); } catch (e) { try { pcApp.resizeCanvas(displayWidth, displayHeight); } catch (e2) {} }
    const c = pcApp.graphicsDevice && pcApp.graphicsDevice.canvas;
    if (c) { c.style.width = `${displayWidth}px`; c.style.height = `${displayHeight}px`; }
  ...
  // 9) attach update hook once
  if (pcApp && !pcApp._gameUpdateAttached) {
    pcApp.on("update", function(dt) {
      try { playcanvasFrameUpdate(dt, performance.now()); } catch (err) { console.error("playcanvasFrameUpdate error:", err); }
    });
...
export function addRemotePlayer(data) {
  ...
  const pcApp = window.playcanvasApp || (typeof pc !== "undefined" && pc.Application && pc.Application.getApplication && pc.Application.getApplication());
  if (!pcApp || !pcApp.root) {
    console.error("addRemotePlayer: PlayCanvas app/root not available");
    return;
  }
  ...
  try {
    ...
    m2.diffuse = new pc.Color(1.0, 0.95, 0.66);
    m2.update();
...
export function attachWeaponToPlayer(playerId, weaponName) {
  ...
  if (!rp) { console.warn("attachWeaponToPlayer: rp not found", playerId); return; }
  const key = (weaponName || "").replace(/-/g,'').toLowerCase();
  const pcApp = PC.app();
  if (!rp.weaponRoot) {
    rp.weaponRoot = new pc.Entity(`rp_weaponRoot_${playerId}`);
    // small offset so weapon sits roughly at hand region
  ...
  // clear previous
  try {
    const children = rp.weaponRoot.children ? rp.weaponRoot.children.slice() : [];
    for (let c of children) {
      try { if (c.parent) c.parent.removeChild(c); } catch {}
      try { c.destroy(); } catch {}
    }
  } catch (e) {}
  ...
  // Prototype store (PlayCanvas entities)
  const proto = (window._pcPrototypeModels || {})[key];
  if (proto && proto.clone) {
    try {
      const clone = proto.clone(true);
      clone.enabled = true;
      rp.weaponRoot.addChild(clone);
      // apply sane per-weapon transform
      switch (key) {
        case "knife": clone.setLocalScale(0.001,0.001,0.001); clone.setLocalEulerAngles(90,180,0); clone.setLocalPosition(0.15, -0.1, 0); break;
        case "deagle": clone.setLocalScale(0.3,0.3,0.3); clone.setLocalEulerAngles(7,180,0); break;
        case "ak47": clone.setLocalScale(0.4,0.4,0.4); clone.setLocalEulerAngles(4,180,0); break;
        case "marshal": clone.setLocalScale(1,1,1); clone.setLocalEulerAngles(0,0,0); break;
        case "m79": clone.setLocalScale(0.5,0.5,0.5); clone.setLocalEulerAngles(7,180,0); break;
        default: break;
      }
      rp.weaponEnt = clone;
      rp.currentWeapon = key;
      ...
      console.warn("attachWeaponToPlayer: prototype clone failed", e);
    }
  }

  // dynamic fallback: load GLB (as in previous code)
  const urlMap = {
  ...
  if (!pcApp || !pcApp.assets || typeof pcApp.assets.loadFromUrl !== "function") return;

  try {
    pcApp.assets.loadFromUrl(url, "container", (err, asset) => {
      if (err) { console.warn("attachWeaponToPlayer load err", err); return; }
      let ent = null;
      try {
        if (asset.resource.instantiateRenderEntity) ent = asset.resource.instantiateRenderEntity();
        else if (asset.resource.instantiateModelEntity) ent = asset.resource.instantiateModelEntity();
        else if (asset.resource.instantiate) ent = asset.resource.instantiate();
      } catch (e) { console.warn("instantiate fallback failed", e); }

      if (!ent) return;
      rp.weaponRoot.addChild(ent);
      // apply transforms similar to above
      switch (key) {
        case "knife": ent.setLocalScale(0.001,0.001,0.001); ent.setLocalEulerAngles(90,160,0); ent.setLocalPosition(0.5,-0.1,-0.7); break;
        case "deagle": ent.setLocalScale(0.5,0.5,0.5); ent.setLocalEulerAngles(7,180,0); break;
        case "legion": ent.setLocalScale(0.5,0.5,0.5); ent.setLocalEulerAngles(7,180,0); break;
        case "ak47": ent.setLocalScale(0.4,0.4,0.4); ent.setLocalEulerAngles(4,180,0); break;
        case "viper": ent.setLocalScale(0.4,0.4,0.4); ent.setLocalEulerAngles(4,180,0); break;
        case "marshal": ent.setLocalScale(1,1,1); ent.setLocalEulerAngles(0,0,0); break;
        case "m79": ent.setLocalScale(0.5,0.5,0.5); ent.setLocalEulerAngles(7,180,0); break;
        default: break;
      }
      rp.weaponEnt = ent;
      rp.currentWeapon = key;
    ...
    console.warn("attachWeaponToPlayer dynamic load failed", e);
  }
}
